...
#define REC_IOCONTROL_REMOTESTATE_SENSORSTATE_KEY "REC_IOCONTROL_REMOTESTATE_SENSORSTATE_KEY"
	/**
	@brief The SensorState is the collection of all sensor values received by the Robotino.
	*/
class SensorState {
	public:
	/*
	...
	*/
		//Velocity/Position
		float actualVelocity[4];
		int actualPosition[4];
		float motorCurrent[4];
		unsigned short rawMotorCurrent[4];
	...
	inline void toQDSAProtocol( unsigned char* buffer ) const
	{
		...
		*(buffer+71) = *(uint8p);
		...
		//slave 3
		*(buffer+77) = ((rawMotorCurrent[3] >> 2) & 0xFF);
		*(buffer+85) = (rawMotorCurrent[3] & 0x3);

		if(avint[3] >= 0)
		{
			*(buffer+87) = 0x1;
			*(buffer+88) = static_cast<unsigned char>(avint[3]);
		}
		else
		{
			*(buffer+88) = static_cast<unsigned char>(-avint[3]);
	...
	inline bool fromQDSAProtocol( const unsigned char* data )
	{
		...
		*uint8p = *(data+71);
		...
		rawMotorCurrent[3] = ( *(data+77) << 2);
		rawMotorCurrent[3] |= ( *(data+85) & 0x3 );
		...
		uint8p = reinterpret_cast<unsigned char*>( &actualPosition[3]);
		*uint8p++ = *(data+89);
		*uint8p++ = *(data+90);
		*uint8p++ = *(data+91);
		*uint8p = *(data+92);
		actualPosition[3] = -actualPosition[3];
		...
		for( unsigned int i=0; i<4; ++i )
		{
			actualVelocity[i] = /*27.0f*/ 900 * 60 * avint[i]; // 900 * 60 / 2000 = 27
